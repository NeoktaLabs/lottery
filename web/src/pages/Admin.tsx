  ...
  if (!isOwner) return (
    <div className="min-h-screen pt-24 px-4 flex justify-center">
      <div className="bg-red-50 p-8 rounded-3xl text-center border border-red-100 max-w-md">
        ...
        <button onClick={onBack} className="mt-6 px-6 py-2 bg-white text-red-700 font-bold rounded-xl border border-red-200">Back</button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen pt-24 pb-20 px-4 animate-fade-in-up">
       <div className="max-w-5xl mx-auto">
         <div className="flex items-center gap-4 mb-8">
           ...
           <div><h1 className="text-3xl font-black text-gray-800 uppercase">Admin Console</h1></div>
         </div>
             ...
             {activeSection === 'config' && (
               <div className="bg-white rounded-3xl p-8 border border-gray-200 shadow-xl space-y-8">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div><label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 ml-1">Fee Recipient</label><input type="text" value={feeRecipient} onChange={(e) => setFeeRecipient(e.target.value)} className="w-full bg-gray-50 border-2 border-gray-100 focus:border-blue-500 text-gray-800 font-mono text-sm rounded-xl px-4 py-3 outline-none" placeholder="0x..." /></div>
                   <div><label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 ml-1">Protocol Fee %</label><input type="number" value={feePercent} onChange={(e) => setFeePercent(e.target.value)} className="w-full bg-gray-50 border-2 border-gray-100 focus:border-blue-500 text-gray-800 font-bold text-lg rounded-xl px-4 py-3 outline-none" placeholder="5" max="20" /></div>
                 </div>
