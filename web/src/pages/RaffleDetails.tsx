...
export function RaffleDetails({ onBack, raffleAddress }: { onBack: () => void, raffleAddress?: string }) {
  ...
  // FIX A: Normalize status to Number immediately so checks like status === 1 work
  ...
  // FIX C: Read entropy settings dynamically from this specific lottery
  ...
  // FIX C: Get fee from the specific entropy contract used by this lottery
  ...
  const execute = (fn: string, args: any[] = [], value: bigint = BigInt(0)) => {
    setTxStep('processing');
    writeContract({ address: raffleAddress as `0x${string}`, abi: LOTTERY_ABI, functionName: fn, args, value });
  };
  ...
  return (
    <div className="min-h-screen pt-20 pb-20 px-4 animate-fade-in-up">
      <div className="max-w-4xl mx-auto mb-6 flex items-center gap-4">
        <button onClick={onBack} className="bg-white/50 p-2.5 rounded-full"><ArrowRight size={20} className="rotate-180" /></button>
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-[2.5rem] p-8 text-center border-4 border-dashed border-amber-200">
           ...
           <div className="grid grid-cols-2 gap-4 text-left">
             <div className="bg-gray-50 p-3 rounded-xl"><div className="text-xs font-bold text-gray-400">My Tickets</div><div className="text-lg font-black">{myTickets?.toString() || '0'}</div></div>
             <div className="bg-gray-50 p-3 rounded-xl">
               <div className="text-xs font-bold text-gray-400">Status</div>
               {/* FIX A: Using Normalized Status */}
             ...
             <div className="space-y-6">
               <div className="bg-gray-50 p-4 rounded-2xl flex justify-between items-center">
                 ...
                 <div className="flex gap-4 items-center">
                   <button onClick={() => setTicketCount(Math.max(1, ticketCount - 1))} className="w-8 h-8 bg-white rounded-full shadow font-bold">-</button>
                   <span className="text-xl font-black">{ticketCount}</span>
                   <button onClick={() => setTicketCount(ticketCount + 1)} className="w-8 h-8 bg-white rounded-full shadow font-bold">+</button>
                 </div>
               </div>
               <div className="flex justify-between px-2 text-sm font-bold text-gray-400">
                 <span>Cost</span>
                 <span className="text-gray-800 text-xl">{ticketPrice ? formatUnits(ticketPrice * BigInt(ticketCount), 6) : '0'} USDC</span>
               </div>
               {needsApproval ? (
                 <button onClick={handleApprove} className="w-full bg-blue-500 text-white font-black py-4 rounded-xl shadow-lg flex justify-center gap-2"><ShieldCheck/> APPROVE USDC</button>
               ) : (
                 <button onClick={() => execute('buyTickets', [BigInt(ticketCount)])} className="w-full bg-amber-500 text-white font-black py-4 rounded-xl shadow-lg flex justify-center gap-2"><Ticket/> BUY TICKETS</button>
               )}
             </div>
           )}

           {/* 2. DRAW (With Fee Safety) */}
           {canDraw && (
             <div className="text-center">
               <h3 className="text-xl font-bold text-gray-800 mb-2">Raffle Ready to Draw</h3>
               <p className="text-sm text-gray-500 mb-6">Time to pick a winner! Anyone can trigger this.</p>
               
               {/* FIX B: Disable if fee not loaded */}
               <button 
                 onClick={() => execute('finalize', [], drawFee || BigInt(0))}
                 disabled={!drawFee} 
               ...
               <p className="text-xs text-gray-400 mt-2">Requires ~{drawFee ? formatUnits(drawFee, 18) : '...'} XTZ fee</p>
             </div>
           )}

           {/* 3. CLAIMS */}
           {(Number(claimableFunds) > 0 || Number(claimableNative) > 0) && (
             <div className="space-y-4">
               <div className="bg-green-50 p-4 rounded-2xl border border-green-100">
                 <h4 className="font-bold text-green-800 mb-2">You have claims!</h4>
                 {Number(claimableFunds) > 0 && <div className="text-sm">💰 {formatUnits(claimableFunds || BigInt(0), 6)} USDC</div>}
                 {Number(claimableNative) > 0 && <div className="text-sm">⚡ {formatUnits(claimableNative || BigInt(0), 18)} XTZ</div>}
               </div>
               {Number(claimableFunds) > 0 && <button onClick={() => execute('withdrawFunds')} className="w-full bg-green-500 text-white font-black py-3 rounded-xl shadow-lg">WITHDRAW USDC</button>}
               {Number(claimableNative) > 0 && <button onClick={() => execute('withdrawNative')} className="w-full bg-green-600 text-white font-black py-3 rounded-xl shadow-lg">WITHDRAW XTZ</button>}
             </div>
           )}

           {/* 4. REFUND */}
           {status === 4 && Number(myTickets) > 0 && Number(claimableFunds) === 0 && (
             <div className="text-center">
               ...
               <button onClick={() => execute('claimTicketRefund')} className="w-full bg-gray-500 text-white font-black py-4 rounded-xl shadow-lg flex justify-center gap-2"><Undo2/> APPLY FOR REFUND</button>
             </div>
           )}
