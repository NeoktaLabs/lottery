...
import { ArrowRight, Ticket, Trophy, ShieldCheck, Loader2, Gavel, Undo2, Banknote } from 'lucide-react';
...
export function RaffleDetails({ onBack, raffleAddress }: { onBack: () => void, raffleAddress?: string }) {
  ...
  // Removed unused txStep variable to fix TS6133
  ...
  // Specific handlers to avoid TS errors
  const handleBuy = () => writeContract({ 
  ...
  const handleDraw = () => writeContract({ 
    address: raffleAddress as `0x${string}`, 
    abi: LOTTERY_ABI, 
  ...
  const handleWithdrawFunds = () => writeContract({ 
    address: raffleAddress as `0x${string}`, 
    abi: LOTTERY_ABI, 
    functionName: 'withdrawFunds' 
  });

  const handleWithdrawNative = () => writeContract({ 
    address: raffleAddress as `0x${string}`, 
    abi: LOTTERY_ABI, 
    ...
    <div className="min-h-screen pt-20 pb-20 px-4 animate-fade-in-up">
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-[2.5rem] p-8 shadow-xl flex flex-col justify-center relative">
           ...
           {status === 1 && !canDraw && (<div className="space-y-6"><div className="bg-gray-50 p-4 rounded-2xl flex justify-between items-center"><span className="font-bold text-gray-500">Tickets</span><div className="flex gap-4 items-center"><button onClick={() => setTicketCount(Math.max(1, ticketCount - 1))} className="w-8 h-8 bg-white rounded-full shadow font-bold">-</button><span className="text-xl font-black">{ticketCount}</span><button onClick={() => setTicketCount(ticketCount + 1)} className="w-8 h-8 bg-white rounded-full shadow font-bold">+</button></div></div><div className="flex justify-between px-2 text-sm font-bold text-gray-400"><span>Cost</span><span className="text-gray-800 text-xl">{ticketPrice ? formatUnits(ticketPrice * BigInt(ticketCount), 6) : '0'} USDC</span></div>{needsApproval ? (<button onClick={handleApprove} className="w-full bg-blue-500 text-white font-black py-4 rounded-xl shadow-lg flex justify-center gap-2"><ShieldCheck/> APPROVE USDC</button>) : (<button onClick={handleBuy} className="w-full bg-amber-500 text-white font-black py-4 rounded-xl shadow-lg flex justify-center gap-2"><Ticket/> BUY TICKETS</button>)}</div>)}
           {canDraw && (<div className="text-center"><h3 className="text-xl font-bold text-gray-800 mb-2">Raffle Ready to Draw</h3><p className="text-sm text-gray-500 mb-6">Time to pick a winner! Anyone can trigger this.</p><button onClick={handleDraw} disabled={!drawFee} className="w-full bg-purple-500 disabled:bg-gray-300 text-white font-black py-4 rounded-xl shadow-lg flex justify-center gap-2"><Gavel/> {drawFee ? "DRAW WINNER" : "Loading Fee..."}</button><p className="text-xs text-gray-400 mt-2">Requires ~{drawFee ? formatUnits(drawFee, 18) : '...'} XTZ fee</p></div>)}
           {(Number(claimableFunds) > 0 || Number(claimableNative) > 0) && (<div className="space-y-4"><div className="bg-green-50 p-4 rounded-2xl border border-green-100"><h4 className="font-bold text-green-800 mb-2">You have claims!</h4>{Number(claimableFunds) > 0 && <div className="text-sm">💰 {formatUnits(claimableFunds || BigInt(0), 6)} USDC</div>}{Number(claimableNative) > 0 && <div className="text-sm">⚡ {formatUnits(claimableNative || BigInt(0), 18)} XTZ</div>}</div>{Number(claimableFunds) > 0 && <button onClick={handleWithdrawFunds} className="w-full bg-green-500 text-white font-black py-3 rounded-xl shadow-lg">WITHDRAW USDC</button>}{Number(claimableNative) > 0 && <button onClick={handleWithdrawNative} className="w-full bg-green-600 text-white font-black py-3 rounded-xl shadow-lg">WITHDRAW XTZ</button>}</div>)}
           {status === 4 && Number(myTickets) > 0 && Number(claimableFunds) === 0 && (<div className="text-center"><h3 className="text-xl font-bold text-gray-800 mb-2">Raffle Canceled</h3><p className="text-sm text-gray-500 mb-6">This raffle was canceled. You can claim a full refund.</p><button onClick={handleRefund} className="w-full bg-gray-500 text-white font-black py-4 rounded-xl shadow-lg flex justify-center gap-2"><Undo2/> APPLY FOR REFUND</button></div>)}
