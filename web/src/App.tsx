                    ...
                    return (
                        <>
                            ...
                            <div className="flex items-center gap-4">
                                <div className="flex items-center gap-3 bg-gray-50/80 p-1.5 pr-2 rounded-2xl border border-gray-200/60 shadow-inner"><button onClick={onOpenCashier} className="bg-amber-500 hover:bg-amber-600 text-white p-2 md:px-4 md:py-2.5 rounded-xl font-bold shadow-sm active:shadow-none active:translate-y-1 transition-all flex items-center gap-2 text-xs md:text-sm h-full"><Store size={18} /><span className="hidden md:inline">Cashier</span></button></div>
...
const HomeRaffleFetcher = ({ address, onNavigate, color, rank }: { address: string, onNavigate: (id: string) => void, color: any, rank?: number }) => {
  ...
  return (
    <div onClick={() => onNavigate(address)} className="cursor-pointer">
      <RaffleCard address={address} deployer={deployer as string} title={name as string} prize={fmtUSDC(prize as bigint)} ticketPrice={fmtUSDC(price as bigint)} sold={Number(sold)} total={Number(minTickets) * 2} endsIn={getEndsInString(deadline as bigint)} color={color} creator={`Player ...${(creator as string).slice(-4)}`} rank={rank} />
    </div>
  );
};
