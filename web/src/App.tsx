...
function SectionHeader({ icon: Icon, title, colorClass }: any) { 
  return (
    <div className="flex items-center gap-3 mb-4 pl-1">
      <div className={`p-2 rounded-xl ${colorClass} text-white shadow-md rotate-[-6deg]`}>
        <Icon size={20} strokeWidth={3} />
      </div>
      <h2 className="text-2xl font-black text-gray-800/90 tracking-tight uppercase drop-shadow-sm">{title}</h2>
    </div>
  ); 
}

function PageModal({ children, onClose }: { children: React.ReactNode, onClose: () => void }) { 
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center animate-fade-in">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose}></div>
      <div className="relative w-full h-full overflow-y-auto overflow-x-hidden pt-4 pb-20">
        <button onClick={onClose} className="fixed top-6 right-6 z-[70] bg-white text-gray-800 p-3 rounded-full shadow-lg hover:bg-gray-100 hover:scale-110 transition-all border border-gray-200">
    ...
    return (
      <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 animate-fade-in-up border border-gray-200">
          <div className="flex justify-center mb-4 bg-amber-100 w-20 h-20 rounded-full items-center mx-auto"><AlertTriangle size={40} className="text-amber-500"/></div>
          <h2 className="text-2xl font-black text-center mb-2 text-gray-800 uppercase">Welcome to Ppopgi</h2>
          <p className="text-center text-gray-500 text-sm mb-6 leading-relaxed">By entering, you confirm you are 18+ years of age and accept the risks associated with blockchain gaming.</p>
          <button onClick={() => {sessionStorage.setItem('ppopgi_disclaimer_accepted', 'true'); setIsOpen(false)}} className="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-4 rounded-xl transition-colors shadow-lg shadow-amber-200">Enter Park</button>
        </div>
      </div>
    );
}
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in-up">
          <div className="bg-[#FFD700] p-5 text-center relative">
            <button onClick={onClose} className="absolute top-4 right-4 text-amber-900 hover:text-white transition-colors"><X size={24} /></button>
            <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto mb-2 shadow-md"><Store size={32} className="text-amber-500" /></div>
            <h2 className="text-xl font-black text-amber-900 uppercase tracking-tight">Coin Cashier 🏪</h2>
          </div>
          <div className="p-6 space-y-4">
            {/* UPDATED COPY FOR TRUST */}
            <div className="text-center text-sm text-gray-600 mb-4 bg-gray-50 p-3 rounded-xl border border-gray-200">
                ...
                <p>Energy is used to power the park. If Energy can't be returned instantly after a game, <span className="font-bold text-amber-600">it’s always saved for you to Collect later</span> in your Dashboard.</p>
            </div>
            
            <div className="flex items-start gap-3 p-3 bg-amber-50 rounded-xl border border-amber-100">
              <div className="bg-amber-100 p-2 rounded-lg text-amber-600"><Coins size={24} /></div>
              <div><h4 className="font-bold text-amber-900 text-sm">Entry Coins (USDC)</h4></div>
            </div>
            <div className="flex items-start gap-3 p-3 bg-green-50 rounded-xl border border-green-100">
              <div className="bg-green-100 p-2 rounded-lg text-green-600"><Zap size={24} /></div>
              <div><h4 className="font-bold text-green-900 text-sm">Energy Coins (XTZ)</h4></div>
            </div>
          </div>
        </div>
      </div>
...
function Navbar({ onOpenCashier, onNavigate }: { onOpenCashier: () => void, onNavigate: (view: any) => void }) {
    ...
    const formatBalance = (value: unknown, decimals: number, symbol: string) => { 
      if (value === undefined || value === null) return `... ${symbol}`; 
      const val = value as bigint; 
      const formatted = parseFloat(formatUnits(val, decimals)).toLocaleString(undefined, { maximumFractionDigits: 2 }); 
                        ...
                        <>
                            <div className="flex items-center gap-6">
                              ...
                              <div className="hidden md:flex items-center gap-2">
                                <button onClick={() => onNavigate('explore')} className="flex items-center gap-1.5 px-4 py-2 rounded-full font-bold text-sm text-gray-600 hover:text-blue-600 hover:bg-blue-50 transition-colors"><Compass size={16} /> Explore</button>
                                {connected && (<button onClick={() => onNavigate('profile')} className="flex items-center gap-1.5 px-4 py-2 rounded-full font-bold text-sm text-gray-600 hover:text-purple-600 hover:bg-purple-50 transition-colors animate-fade-in"><LayoutDashboard size={16} /> Dashboard</button>)}
                                {connected && isOwner && (<button onClick={() => onNavigate('admin')} className="flex items-center gap-1.5 px-4 py-2 rounded-full font-bold text-sm text-gray-600 hover:text-red-600 hover:bg-red-50 transition-colors animate-fade-in"><Shield size={16} /> Admin</button>)}
                                <button onClick={() => connected ? onNavigate('create') : openConnectModal()} className="flex items-center gap-1.5 px-4 py-2 rounded-full font-bold text-sm text-gray-600 hover:text-amber-700 hover:bg-amber-100 transition-colors"><Ticket size={16} /> Create</button>
                              </div>
                            </div>
                                ...
                                <div className="flex items-center gap-3 bg-gray-50/80 p-1.5 pr-2 rounded-2xl border border-gray-200/60 shadow-inner">
                                  {connected && (
                                    <div className="hidden lg:flex flex-col gap-1 pl-1">
                                      <div className="w-36 bg-[#E8F5E9] text-green-700 px-2.5 py-1 rounded-md font-bold text-[10px] flex items-center justify-between border border-green-200 shadow-sm tracking-tight"><div className="flex items-center gap-1.5"><Zap size={10} className="fill-green-600" /> <span>Energy</span></div><span>{xtzBalance ? `${parseFloat(xtzBalance.formatted).toFixed(2)} XTZ` : '...'}</span></div>
                                      <div className="w-36 bg-[#FFF8E1] text-amber-700 px-2.5 py-1 rounded-md font-bold text-[10px] flex items-center justify-between border border-amber-200 shadow-sm tracking-tight"><div className="flex items-center gap-1.5"><Coins size={10} className="fill-amber-500" /> <span>Entry</span></div><span>{formatBalance(usdcBalance, 6, 'USDC')}</span></div>
                                    </div>
                                  )}
...
// UPDATED: Now fetches min/max tickets to pass to RaffleCard for correct progress logic
const HomeRaffleFetcher = ({ address, onNavigate, color, rank }: { address: string, onNavigate: (id: string) => void, color: any, rank?: number }) => {
  ...
  const { data: maxTickets } = useReadContract({ ...contractConfig, functionName: 'maxTickets' }); // ADDED
  ...
  return (
    <div onClick={() => onNavigate(address)} className="cursor-pointer">
      <RaffleCard 
        ...
        sold={Number(sold)}
        minTickets={Number(minTickets)} // ADDED
        maxTickets={Number(maxTickets)} // ADDED
  ...
  return (
    <main className="container mx-auto px-4 pt-20 max-w-[100rem] animate-fade-in">
      ...
      <div className="w-fit mx-auto bg-white/10 backdrop-blur-sm rounded-3xl p-6 mb-6 border border-white/30 shadow-lg relative overflow-visible mt-8">
          <SectionHeader icon={Sparkles} title="Fresh Drops" colorClass="bg-yellow-400" />
          {latestRaffles.length > 0 ? (
            <div className="flex flex-col md:flex-row items-center md:items-end justify-center gap-6 md:gap-4 pt-4 md:pt-8 pb-4 relative z-10">
              {latestRaffles[1] && <div className="w-full max-w-[260px] order-2 md:order-1 relative flex flex-col items-center"><HomeRaffleFetcher address={latestRaffles[1]} onNavigate={onNavigate} color="silver" rank={2} /><div className="hidden md:block w-[90%] h-6 bg-gradient-to-t from-slate-200 to-slate-100/50 rounded-b-2xl mt-[-10px] z-0 border-b-2 border-slate-200"></div></div>}
        ...
        <RainbowKitProvider coolMode>
          <div className="min-h-screen pb-12 relative">
            <Navbar 
              ...
              onNavigate={(view) => { if (view === 'home') closeModal(); else setCurrentModal(view); }} 
            />
            ...
            <div className={`transition-all duration-300 ${currentModal !== 'none' ? 'scale-[0.98] blur-[2px] opacity-50 pointer-events-none' : ''}`}>
              <HomeView onNavigate={openDetails} />
            </div>
