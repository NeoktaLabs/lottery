...
const HomeRaffleFetcher = ({ address, onNavigate, color, rank }: { address: string, onNavigate: (id: string) => void, color: any, rank?: number }) => {
  ...
  return (
    <div onClick={() => onNavigate(address)} className="cursor-pointer">
      <RaffleCard address={address} deployer={deployer as string} title={name as string} prize={fmtUSDC(prize as bigint)} ticketPrice={fmtUSDC(price as bigint)} sold={Number(sold)} minTickets={Number(minTickets)} maxTickets={Number(maxTickets)} endsIn={getEndsInString(deadline as bigint)} color={color} creator={`Player ...${(creator as string).slice(-4)}`} rank={rank} />
    </div>
  );
};
